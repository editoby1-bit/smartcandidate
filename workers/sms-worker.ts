...
const worker = new Worker<SendJobData>(
  ...
  async (job) => {
    ...
    await db.from('sends').update({ status: 'sending', attempted_at: new Date().toISOString() }).eq('id', send_id)
    const result = await sendSMS(phone, message, process.env.AT_SENDER_ID)
    if (result.status === 'success') {
      await db.from('sends').update({ status: 'sent', message_id: result.messageId }).eq('id', send_id)
      await db.rpc('increment_campaign_sent', { campaign_id_arg: campaign_id })
      console.log(`[SMS] ✓ ${phone}`)
    } else {
      await db.from('sends').update({ status: 'failed', error_message: result.error }).eq('id', send_id)
      await db.rpc('increment_campaign_failed', { campaign_id_arg: campaign_id })
      throw new Error(result.error)
    }
  },
  { connection, concurrency: 50, limiter: { max: 500, duration: 10_000 } }
)
