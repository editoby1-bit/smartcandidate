...
export async function sendSMSBulk(
  phones: string[],
  defaultMessage: string,
  individualMessages?: { phone: string; message: string }[]
): Promise<{ sent: number; failed: number; recipients: any[] }> {
  try {
    // If individual messages differ, send one by one in parallel (personalisation)
    // If same message, send as true bulk (much faster)
    const allSame = !individualMessages || 
      individualMessages.every(m => m.message === defaultMessage)

    if (allSame) {
      // True bulk — one API call for all numbers
      const body = new URLSearchParams({
        username: process.env.AT_USERNAME!,
        to:       phones.join(','),
        message:  defaultMessage,
      ...
      const text = await res.text()
      console.log(`[AT BULK] ${phones.length} numbers — ${text.substring(0, 150)}`)
      ...
      const failed = recipients.filter((r: any) => r.status !== 'Success').length
      return { sent, failed, recipients }

    } else {
      // Personalised — parallel sends with concurrency limit
      const CONCURRENCY = 20
      const allRecipients: any[] = []
      let sent = 0, failed = 0
